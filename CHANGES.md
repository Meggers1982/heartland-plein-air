...
### "Last updated" dates are derived from git, not hardcoded

Both dates started as hardcoded strings. They now come from
`src/lib/legalUpdated.json`, regenerated by
`scripts/stamp-legal-updated.mjs` — wired to `prebuild`, so every deploy
re-derives them and editing `Privacy.tsx` or `Terms.tsx` moves *that page's*
date without anyone remembering to. Each page is tracked independently, so
editing the privacy policy does not restamp the terms.

**`new Date()` was deliberately not used.** A build-time timestamp would
restamp both pages on every unrelated deploy, so a legal page would claim it
had been revised today when nothing in it changed. Git commit dates are the
only honest record of when the content actually moved. For the same reason,
when git can't answer (no binary, or a shallow clone whose fetched commits
didn't touch the file) the script keeps the committed value and warns, rather
than falling back to today — verified by running it with `git` removed from
`PATH`.

`LegalPage.tsx` renders the date in a `<time dateTime="YYYY-MM-DD">` element.
Formatting reads the calendar date straight off the ISO string instead of going
through `new Date()`: the stamps carry the committer's UTC offset, so a
late-evening commit (`…T21:40-04:00`) would roll forward a day if converted to
UTC and display a date the change didn't happen on.

`LegalPage.tsx` itself is deliberately **not** tracked — restyling the shared
chrome is not a policy change and must not move either date.

Granularity is one day; second-level drift between the stamp and the commit is
invisible. To refresh the committed fallback by hand: `npm run stamp:legal`.
