...
**The countdown changes with the festival.** The countdown banner and the ribbon under the menu each have three states: the clock before opening, "Happening now" (with a link to that day on `/schedule`) from midnight Central on the first day, and a thank-you after the last day, when the ribbon becomes a link to the online painting sale. The boundaries come from the dates on the Sanity "Festival Dates" document; the during/after copy is in Studio under **Site-wide Text → During the festival / After the festival**, with code fallbacks in `src/lib/festivalPhaseCopy.ts`. Exception: after the festival, the banner's call-to-action card is a hardcoded "Festival paintings are on sale" message linking to the FASO online show (open through Oct 4, 2026), so the "After the festival" newsletter title/body fields in Studio are not currently displayed there. The FASO URL lives in `src/lib/onlineSale.ts` and is also shown, after the festival only, on the Schedule page's Online Sales entry, the nav ribbon, the Gallery header, the homepage hero, and (via `OnlineSaleLink`) the homepage schedule card, three FAQ answers, Open Division, Tickets, About and the footer. CHANGES.md follow-up #10 lists them and how to remove them. Every page renders all three states, and a tiny inline script in the root layout's `<head>` stamps `data-festival-phase` on `<html>` before first paint. The `phase-live:` / `phase-after:` Tailwind variants key off that attribute. This way a prerendered page that's up to an hour old still paints the right state first time, without a flash. Phase logic and its tests are in `src/lib/festivalDate.ts`.
