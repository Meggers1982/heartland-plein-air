...
export type FlatPainting = GalleryPainting & { artistName: string; artistSlug: string; medium: string };

export const allPaintings: FlatPainting[] = galleryArtists.flatMap((a) =>
  a.paintings.map((p) => ({ ...p, artistName: a.name, artistSlug: a.slug, medium: a.medium })),
);
