...
- `siteChrome` ("Site-wide Text") carries the countdown banner/ribbon copy for all three festival phases: the "Countdown" group (before), "During the festival", and "After the festival". The during/after fields fall back to code defaults in `src/lib/festivalPhaseCopy.ts` when empty. Exception: the after-festival online-sale message + FASO link is hardcoded in `CountdownBanner.tsx` (CTA card), `CountdownRibbon.tsx`, `Gallery.tsx` (header), `HeroSection.tsx` (extra button) and `Schedule.tsx`, so `afterNewsletterTitle`/`afterNewsletterBody` are currently unused there — see CHANGES.md follow-up #10
