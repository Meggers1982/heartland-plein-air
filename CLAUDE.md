...
- `siteChrome` ("Site-wide Text") carries the countdown banner/ribbon copy for all three festival phases: the "Countdown" group (before), "During the festival", and "After the festival". The during/after fields fall back to code defaults in `src/lib/festivalPhaseCopy.ts` when empty. Exception: the after-festival CTA card in `CountdownBanner.tsx` (online-sale message + FASO link) is hardcoded, so `afterNewsletterTitle`/`afterNewsletterBody` are currently unused there — see CHANGES.md follow-up #10
