...
- Festival phase: `phase-live:` / `phase-after:` Tailwind variants (defined in `tailwind.config.ts`) style elements by the `data-festival-phase` attribute an inline `<head>` script sets on `<html>` before first paint. Unprefixed classes are the "before" state. Render all variants and let CSS pick — never branch on the phase in React render (see the phase notes in `src/lib/festivalDate.ts`)
...
- `siteChrome` ("Site-wide Text") carries the countdown banner/ribbon copy for all three festival phases: the "Countdown" group (before), "During the festival", and "After the festival". The during/after fields fall back to code defaults in `src/lib/festivalPhaseCopy.ts` when empty. Exception: the after-festival online-sale message + FASO link (`src/lib/onlineSale.ts`, `src/components/OnlineSaleLink.tsx`) is hardcoded in about a dozen places, listed in CHANGES.md follow-up #10, including `CountdownBanner.tsx`'s CTA card, so `afterNewsletterTitle`/`afterNewsletterBody` are currently unused there — see CHANGES.md follow-up #10
